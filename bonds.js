...
.catch((e) => $.logErr(e))
  ...
  //  获取集思录里面的所有可转债信息
function convertibleBond() {
  return new Promise((resolve, reject) => {
    $.get({url: 'https://www.jisilu.cn/webapi/cb/list/'}, (error, response, data) => {
      try {
        // console.log(JSON.stringify(data))
        $.bonds = JSON.parse(data);
        // console.log('11111'+ JSON.parse(JSON.stringify($.bonds.data[0])))
        // console.log($.bonds.data[0]['bond_id'])
        // console.log(JSON.parse(JSON.stringify($.bonds)))
        convertibleBondFormart()
      } catch (e) {
        $.logErr(e, resp);
      } finally {
        resolve()
...
function convertibleBondFormart() {
  var bonds = $.bonds.data
  console.log('ceshi ')
  var bondTitles = []
  suffix = {'110': 'SH', '113': 'SH', '127': 'SZ', '128': 'SZ' , '123': 'SZ'}
  bondTitles.push({title: '转债代码', value: 'bond_id'})
  bondTitles.push({title: '转债名称', value: 'bond_nm'})
  bondTitles.push({title: '最新价', value: 'price'})
  // bondTitles.push({title: '涨跌幅', value: 'increase_rt'})
  // bondTitles.push({title: '正股代码', value: 'stock_cd'})
  // bondTitles.push({title: '(市场)正股代码', value: 'stock_id'})
  // bondTitles.push({title: '正股名称', value: 'stock_nm'})
  // bondTitles.push({title: '正股价格', value: 'sprice'})
  // bondTitles.push({title: '正股涨跌幅', value: 'sincrease_rt'})
  // bondTitles.push({title: '转股价', value: 'convert_price'})
  // bondTitles.push({title: '转股价值', value: 'convert_value'})
  // bondTitles.push({title: '溢价率', value: 'premium_rt'})
  // bondTitles.push({title: '评级', value: 'rating_cd'})
  // bondTitles.push({title: '回售触发价', value: 'put_convert_price'})
  // bondTitles.push({title: '强赎触发价', value: 'force_redeem_price'})
  // bondTitles.push({title: '转债占比', value: 'convert_amt_ratio'})
  // bondTitles.push({title: '到期时间', value: 'short_maturity_dt'})
  // bondTitles.push({title: '剩余时间', value: 'year_left'})
  // bondTitles.push({title: '成交额', value: 'volume'})
  // bondTitles.push({title: '换手率', value: 'turnover_rt'})
  // bondTitles.push({title: '到期收益率', value: 'ytm_rt'})
  bondTitles.push({title: '双低', value: 'dblow'})
  // bondTitles.push({title: 'PB', value: 'pb'})
  // bondTitles.push({title: '波动率', value: 'volatility_rate'})
  // bondTitles.push({title: '纯债价值', value: 'bond_value'})
  // bondTitles.push({title: '基金持仓', value: 'fund_rt'})
  // bondTitles.push({title: '转股时间', value: 'convert_cd_tip'})
  // bondTitles.push({title: '回售起始日', value: 'next_put_dt'})
  // bondTitles.push({title: '到期日', value: 'maturity_dt'})
  // bondTitles.push({title: '发行规模(亿元)', value: 'orig_iss_amt'})
  // bondTitles.push({title: '剩余规模(亿元)', value: 'curr_iss_amt'})
  // bondTitles.push({title: '上市日期', value: 'list_dt'})
  // bondTitles.push({title: '基金持仓', value: 'fund_rt'})
  ...
  var balance = []
  var data = []
  console.log('ceshi ' + bonds.length)
  bonds = filterData(bonds)
...
//  筛选池中的可转债
function filterData(bonds) {
  //  根据双低来 排序
  bonds.sort(function (a,b) {
    return Number(a.dblow) - Number(b.dblow)
  })
  var data = []
  for (var i = 0; i < bonds.length; i++) {
    var bond = bonds[i]
    if(Number(bond['price']) <= 100) {
          data.push(bonds[i])
    }
    if (data.length > 20) {
      break;
